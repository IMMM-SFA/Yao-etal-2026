...
fig = plt.figure(figsize=(5.2, 6))  # Increased height for two subplots
...
for crop in ['corn']:
    ## near future
    df_345c_nf = pd.read_csv(f'{path}sensitivity_analyses/ssmi_threshold_-1/{crop}_financial_loss_nf_3_45_c_2021-2055.csv')
    df_585c_nf = pd.read_csv(f'{path}sensitivity_analyses/ssmi_threshold_-1/{crop}_financial_loss_nf_5_85_c_2021-2055.csv')
    df_345h_nf = pd.read_csv(f'{path}sensitivity_analyses/ssmi_threshold_-1/{crop}_financial_loss_nf_3_45_h_2021-2055.csv')
    df_585h_nf = pd.read_csv(f'{path}sensitivity_analyses/ssmi_threshold_-1/{crop}_financial_loss_nf_5_85_h_2021-2055.csv')
    ...
    ax_bottom.text(x_conus_99 - 15, 0.015, f'{x_conus_99:.0f}', color='black', fontsize=12, ha='left', va='center', weight="bold") # crop_colors[crop],
    ...
    for nca_id in [4, 5, 6]:
        ...
        if nca_id == 6:
            ax_bottom.text(x_nca_99-16 , 0.015, f'{x_nca_99:.0f}', color=region_colors[nca_id], fontsize=12, ha='left', va='center', weight="bold")
        elif nca_id == 4:
            ax_bottom.text(x_nca_99 + 1.5, 0.015, f'{x_nca_99:.0f}', color=region_colors[nca_id], fontsize=12, ha='left', va='center', weight="bold")
        elif nca_id == 5:
            ax_bottom.text(x_nca_99+0.4, 0.015, f'{x_nca_99:.0f}', color=region_colors[nca_id], fontsize=12, ha='left', va='center', weight="bold")   
            
# Final plot settings
ax_top.set_ylim(0.90, 1.0)  # Zoomed-in range for high probabilities
ax_bottom.set_ylim(0, 0.90)  # Lower range to align at y=0.9
ax_top.set_xlim(-50, 150)
ax_bottom.set_xlim(-50, 150)
...
plt.savefig(f'{path}sensitivity_analyses/ssmi_threshold_-1/financial_loss_345diff585_corn_mean.png', dpi=300, bbox_inches='tight')
print ('done')