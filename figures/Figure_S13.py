...
area_df = pd.read_csv('/filepath/sensitivity_analyses/ssmi_threshold_-1/drought_area_by_conus_nca_hist+nearfuture_withclimateonly_timeseries.csv')
intensity_df = pd.read_csv('/filepath/sensitivity_analyses/ssmi_threshold_-1/drought_intensity_by_conus_nca_hist+future_withclimateonly_timeseries.csv')
...
def plot_drought_area_intensity_compact(crop, area_dfs, intensity_dfs, region_abbrev, region_order,
    ...
    for ax, df_dict, title, value_col, label in zip(axes, dfs, titles, value_cols, labels):
        ...
        # Plotting
        bar_width = 0.22  # slightly narrower
    ...
    if add_legend:
        ...
        # 2. Create a real errorbar on a hidden axis and extract its container
        legend_ax = fig.add_axes([0, 0, 0.01, 0.01])  # invisible
        legend_ax.set_xlim(0,1); legend_ax.set_ylim(0,1)
        legend_ax.axis('off')
        
        # Plot a real errorbar
        eb = legend_ax.errorbar(0.5, 0.5, yerr=0.25,
                                color='black', capsize=3, capthick=1.2, lw=1.2,
                                fmt='none')  # no marker
        
        # The full errorbar container (line + caps) is eb itself
        eb_handle = eb
...
plot_drought_area_intensity_compact('corn', area_dfs, intensity_dfs, region_abbrev, region_order,
                                    ...
                                    output_file='/filepath/sensitivity_analyses/ssmi_threshold_-1/corn_2pannel.png')

plot_drought_area_intensity_compact('soybean', area_dfs, intensity_dfs, region_abbrev, region_order,
                                    exclude_regions=['NW', 'SW'],
                                    output_file='/filepath/sensitivity_analyses/ssmi_threshold_-1/soybean_2pannel.png',
                                    add_legend=True)
