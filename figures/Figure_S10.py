...
future_path = '/filepath/sensitivity_analyses/median_nondrought_reference_yield/'
hist_path = '/filepath/sensitivity_analyses/median_nondrought_reference_yield/'
climate_change_only_path = '/filepath/sensitivity_analyses/median_nondrought_reference_yield/'
...
panel_labels = ['(a)', '(b)'] # Adjusted to 2 labels

# Loop strictly over corn and soybean
for idx, crop in enumerate(['corn', 'soybean']):
    ...
    # Create plotting dataframe in correct order
    ...
    # Exclusions for corn and soybean (setting both mean and error to NaN)
    exclude = [7, 8]   # SW, NW are excluded for both corn and soybean
    ...
    # === DEBUG PRINT: Corn - Midwest values ===
    if crop == 'corn':
        mw_row = df_plot[df_plot['nca_id'] == 4]
        if not mw_row.empty:
            clim_mean = mw_row['Climate_mean'].values[0]
            clim_low  = mw_row['Climate_low'].values[0]
            clim_high = mw_row['Climate_high'].values[0]
            lulcc_mean = mw_row['LULCC_mean'].values[0]
            lulcc_low  = mw_row['LULCC_low'].values[0]
            lulcc_high = mw_row['LULCC_high'].values[0]

            print("\n=== CORN - MIDWEST (MW) VALUES ===")
            print(f"Climate change contribution (mean): {clim_mean:+6.2f}%")
            print(f"   → range: {clim_mean - clim_low:+6.2f} to {clim_mean + clim_high:+6.2f}%  "
                  f"(min={clim_mean - clim_low:+6.2f}, max={clim_mean + clim_high:+6.2f})")
            print(f"Land-use change contribution (mean): {lulcc_mean:+6.2f}%")
            print(f"   → range: {lulcc_mean - lulcc_low:+6.2f} to {lulcc_mean + lulcc_high:+6.2f}%  "
                  f"(min={lulcc_mean - lulcc_low:+6.2f}, max={lulcc_mean + lulcc_high:+6.2f})")

    # Axis limit settings optimized by crop type
    if crop == 'corn':
        ax.set_xlim(-12, 55)
    else:  # soybean
        ax.set_xlim(-12, 110)
...
plt.savefig(f'{future_path}production_loss_contribution_2bars_with_range_errorbars_cornsoybean.png',
